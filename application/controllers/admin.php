...
class Admin extends CI_Controller {
    ...
    function __construct() {
        ...
        //$this->config->item('use_mongodb', 'admin_auth') ? $this->load->library('mongo_db') : $this->load->database();
        $this->load->database();
            ...
            // added for checking default password - end
                ...
                if ($this->admin_auth->login(strtoupper($this->input->post('identity')), $this->input->post('password'), $remember)) {
                     ...
                     */
                     ...
                     // if($this->session->userdata['loginname']=="OVESAL4"){
                      if (preg_match('/OVESAL/',$this->session->userdata['loginname'])){
                        //echo"the user is a overseas user <br>";
                        redirect('/overseas/index', 'refresh');
                      }
                      else
                      {
                        if (isset($url)) {
...
//			$this->_render_page('admin/login', $this->data);
    ...
    function idlelogin() {
            ...
            //the user is not logging in so display the login page
            //set the flash data error message if there is one
            $this->data['message'] = "You have been logged out due to Page Idle Time Out..!";
            $this->data['identity'] = array('name' => 'identity',
                'id' => 'identity',
                'type' => 'text',
                'value' => $this->form_validation->set_value('identity'),
...
//          $this->_render_page('admin/login', $this->data);
    ...
    //log the user out
    function logout() {
        $timeout = urldecode($this->uri->segment(3)); 
        
        $this->data['title'] = "Logout";
        if ($timeout!="")
        {
            $this->data['message'] = "You have been logged out due to Idle Time Out..!";
             $user = $this->admin_auth->user()->row();
             $logout = $this->admin_auth->logoutnew($user);
             //$this->load->view('admin/loginnew',$this->data);
             redirect('admin/idlelogin', 'refresh',$this->data);
        }
        else
        {
        $this->data['message'] = "";
    ...
    function change_password() {
        ...
        if (!$this->admin_auth->logged_in()) {
//			redirect('admin/login', 'refresh');
            redirect('admin/loginnew', 'refresh');
        }
    ...
    function edit_user($id) {
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            if (isset($groupData) && !empty($groupData)) 
            {
                ...
                $this->admin_auth->remove_from_group($groupData, $id);
